...
def no_conflicts(csp, assignment,var,a,b):  # make sure team's arent scheduled for 2 games at once
    ...
    while n < len(assignment.items()):
        if a in assignment_items[n][0].assigned_teams:
            if a_count == 1:
                db_print("DOESNT SATISFY NO CONFLICTS ("+str(a)+")")
                return False
            else:
                a_count += 1
        if b in assignment_items[n][0].assigned_teams:
            if b_count == 1:
                db_print("DOESNT SATISFY NO CONFLICTS (" + str(b) + ")")
                return False
            else:
                b_count += 1
        n += 1
...
def each_team_plays(csp, assignment,var,team1,team2):  # make sure each team plays the same amount of games, every week
    ...
    if (len(assignment) == len(csp.variables) or (len(assignment) % (nfields * ntimes) == 0 and len(assignment) != 0)):#if the top of the week
        for team in teams:
...
def has_enough_breaks(var,team1,team2):
    ...
    if (max_break - min_break > 1 and nteams <= 8) or (max_break - min_break > 2):
        return True
    return False
...
def respects_break_scheduling(csp,assignment,var,team1,team2): # makes sure instances with breaks dont violate break rules
    ...
    if ntimes == 3: #do a bunch of other checks to enforce pattern
    ...
    if len(assignment) == ngames:
        ...
        for team, counts in team_time_slot_counters.items():
            ...
            if max_usage - min_usage == 0:
                ...
                db_print("DOESNT RESPECT ALTERNATING TIMES")
                return False
        if nteams <= 8:
            return teams_with_flat_counts >= nteams / 2
...
def main():
    ...
    if games_in_a_week > 1:
        if len(times) > 2:
            constraints = [each_team_plays, no_conflicts, plays_each_team,respects_break_scheduling]
        else:
    ...
    if solution:
        ...
        print(nteams,ntimes,nfields,nweeks,games_in_a_week)
